...
class DropBlock_Ske(nn.Module):
    def __init__(self, num_point=25, keep_prob=0.7):
...
class Simam_Drop(nn.Module):
    def __init__(self, num_point=25, e_lambda=1e-4, keep_prob=0.7):
...
if __name__ == '__main__':
    ...
    drop_sk = Simam_Drop(num_point=25, keep_prob=0.7)
    w = drop_sk(x)
    print(w.shape)